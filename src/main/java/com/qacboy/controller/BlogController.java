...
import com.qacboy.repository.BlogRepository;
...
//            modelMap.addAttribute("updateBlog",0);
            return "admin/blogs";
        }else {
            blogList = blogRepository.findByUserId(MainController.uid);
//            String updateUser="1";
            if (!blogList.isEmpty()) {
                modelMap.addAttribute("blogList", blogList);
//                modelMap.addAttribute("updateUser",updateUser);
    ...
    public String addBlog(ModelMap modelMap) {
        UserEntity user= userRepository.findOne(MainController.uid);
        // 向jsp注入用户列表
